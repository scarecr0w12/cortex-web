...
| `:\(\)\{.*\}` | Fork bomb patterns (simplified regex example; production uses AST-based analysis) |
...
vaultStore(name, value):
  ...
  key = PBKDF2(passphrase, salt, 200000, HMAC-SHA-256) → AES-256 key
  iv = crypto.getRandomValues(12 bytes)
  ciphertext = AES-GCM-256.encrypt(iv, key, value)
  store(salt || iv || ciphertext) in vault_entries

vaultGet(name):
  buf = vault_entries[name].encrypted_data
  salt = buf[0:16]; iv = buf[16:28]; cipher = buf[28:]
  key = PBKDF2(passphrase, salt, 200000, HMAC-SHA-256) → AES-256 key
...
- PBKDF2 with 200,000 iterations of HMAC-SHA-256 for key derivation
...
| Policy Rules | Pre-seeded rules blocking known dangerous patterns |
